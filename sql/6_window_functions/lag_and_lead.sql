# both are non-aggregate
# VALUE window functions return a value that can be found in the database
# LAG returns the value from a specified field of a record that precedes the current row

# 1: Write a query that can extract the following information from the "employees" database:
...
# 2: The MySQL LAG() and LEAD() value window functions can have a second argument, designating how many rows/steps back (for LAG()) or forth (for LEAD()) we'd like to refer to with respect to a given record.
